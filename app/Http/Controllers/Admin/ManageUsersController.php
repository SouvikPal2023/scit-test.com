...
class ManageUsersController extends Controller
{
    ...
    public function detail($id)
    {
        ...
        $State = State::where('country_id',$user->address->country)->get();
        $City = City::where('state_id',$user->address->state)->get();
        return view('admin.users.detail', compact('page_title', 'user','totalDeposit','totalTransaction','countries','State','City'));
    }
    ...
    public function delete($id){
        ...
        $oldimg = $user->image;
        $user->delete();
        //unlink(public_path('assets/templates/basic/images/client/'.$oldimg));
        if($user) {
    ...
    public function sendEmailSingle(Request $request, $id)
    {
        ...
        $user = User::findOrFail($id);
        send_general_email($user->email, $request->subject, $request->message, $user->username1);
